// frontend/src/utils/generateWebsiteContent.js

/**
 * Generates a complete HTML storefront page dynamically for a given store.
 * @param {Object} store - The store object (name, description, etc.)
 * @param {Array} publishedProducts - The list of published WebProducts from backend
 * @returns {string} - Full HTML string for that store
 */

export const generateWebsiteContent = (store, publishedProducts = []) => {
  console.log(`🧱 Generating website for store: ${store?.name}, products: ${publishedProducts?.length}`);

  // HERO SECTION
  const heroSection = `
    <section class="hero-gradient text-white py-20" data-gjs-type="hero-block">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-4xl font-bold mb-4">Welcome to ${store?.name || "Our Store"}</h2>
        <p class="text-xl mb-8">${store?.description || `Your trusted ${store?.category || "online"} store with amazing products!`}</p>
  ...
  const productsGrid =
    !publishedProducts || publishedProducts.length === 0
      ...
      : `
        <section id="products" class="py-16 bg-gray-50" data-gjs-type="product-grid">
  ...
  const footerSection = `
    <footer class="bg-gray-900 text-white py-12" data-gjs-type="footer-block">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8">
        <div>
          <h3 class="text-lg font-semibold mb-4">${store?.name || "Our Store"}</h3>
        ...
        <div>
          ...
          <div class="text-gray-400 space-y-2">
            <p>📧 info@${(store?.name || "store").toLowerCase().replace(/\s+/g, "")}.com</p>
            <p>📞 +1 (555) 123-4567</p>
  ...
  <head>
    ...
    <title>${store?.name || "My Store"} - Online Store</title>
