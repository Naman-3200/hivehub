...
const SuperAdminDashboard = () => {
  ...
  useEffect(() => { fetchOverview(); fetchSubs(); }, []);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <SCard label="Total Users" value={overview.totalUsers} />
        <SCard label="Total Stores" value={overview.totalStores} />
        <SCard label="HiveeHub Revenue" value={`₹${(overview.totalRevenueHiveeHub || 0).toFixed(2)}`} />
        <SCard label="Active Subs" value={overview.subscriptions.active} />
      </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-left p-2">Latest Amount</th>
              </tr>
            </thead>
            <tbody>
              {subs.map(u => (
                <tr key={u._id} className="border-t">
                  ...
                  <td className="p-2">{u.subscription?.expiresAt ? new Date(u.subscription.expiresAt).toLocaleString() : "-"}</td>
                  <td className="p-2">₹{(u.subscription?.latestAmount || 0).toFixed(2)}</td>
                </tr>
              ))}
              {subs.length === 0 && <tr><td className="p-4 text-center text-gray-500" colSpan={6}>No subscriptions</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
