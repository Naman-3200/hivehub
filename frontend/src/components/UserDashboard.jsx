...
const searchCJProducts = async (query) => {
  ...
  setError(null);
  try {
    const res = await fetch(`https://hivehub-1.onrender.com/api/products/search?q=${encodeURIComponent(query)}`);
...
const fetchProducts = async (page = 1, size = 200) => {
  ...
  try {
    const res = await fetch(
      `https://hivehub-1.onrender.com/api/products?pageNum=${page}&pageSize=${size}`
    );
    if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
    const data = await res.json();
  ...
  try {
    ...
    const res = await fetch(`https://hivehub-1.onrender.com/api/my-products/${productId}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
